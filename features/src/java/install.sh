...
sdk_install() {
    ...
    if [ "${requested_version}" = "latest" ] || [ "${requested_version}" = "lts" ] || [ "${requested_version}" = "default" ]; then
        ...
        local regex="${prefix}[0-9]+\\.?[0-9]*\\.?[0-9]*${suffix}"
        local version_list=$(su ${USERNAME} -s /bin/bash -c "source \${SDKMAN_DIR}/bin/sdkman-init.sh && sdk list ${install_type} 2>&1 | grep -oE \"${regex}\" | tr -d ' ' | sort -rV")
    ...
    su ${USERNAME} -s /bin/bash -c "umask 0002 && . ${SDKMAN_DIR}/bin/sdkman-init.sh && sdk install ${install_type} ${requested_version} && sdk flush archives && sdk flush temp"
}
...
if [ ! -z "${ADDITIONAL_VERSIONS}" ]; then
    ...
    IFS=$OLDIFS
    su ${USERNAME} -s /bin/bash -c ". ${SDKMAN_DIR}/bin/sdkman-init.sh && sdk default java ${JAVA_VERSION}"
fi
