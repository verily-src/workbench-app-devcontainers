...
JUPYTER_USER="${JUPYTERUSER:-"jupyter"}"
...
if [ "${INSTALL_JUPYTERLAB}" = "true" ]; then
    ...
    if [ "$(id -u)" -eq 0 ] && [ "$JUPYTER_USER" != "root" ]; then
    ...
    if [ "$INSTALL_UNDER_ROOT" = false ]; then
        ...
        SUDOERS_FILE="/etc/sudoers.d/$JUPYTER_USER"
        SEARCH_STR="Defaults secure_path="
        REPLACE_STR="Defaults secure_path=/home/${JUPYTER_USER}/.local/bin"
    ...
    if [ -n "${CONFIGURE_JUPYTERLAB_ALLOW_ORIGIN}" ]; then
        ...
        if [ "$INSTALL_UNDER_ROOT" = false ]; then
            CONFIG_DIR="/home/$JUPYTER_USER/.jupyter"
        fi
