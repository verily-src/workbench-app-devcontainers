...
# SUBDIR can be passed as first argument to specify a subdirectory within the workspace
# e.g., ./copy-csv-files.sh "verily/dry_run" would look in /home/shiny/workspace/verily/dry_run
readonly SUBDIR="${1:-}"
readonly BASE_SOURCE_DIR="/home/shiny/workspace"
if [[ -n "${SUBDIR}" ]]; then
    readonly SOURCE_DIR="${BASE_SOURCE_DIR}/${SUBDIR}"
else
    readonly SOURCE_DIR="${BASE_SOURCE_DIR}/"
fi
