      ...
      <div style={{
        ...
        {isLoading ? (
          ...
          <>
            {/* Quality Score */}
            <div style={{
              backgroundColor: qualityData.overall_score >= 90 ? '#f0fdf4' : '#fef3c7',
              border: `2px solid ${qualityData.overall_score >= 90 ? '#86efac' : '#fbbf24'}`,
              borderRadius: '8px',
              padding: '20px',
              marginBottom: '24px',
              textAlign: 'center'
              ...
              <div style={{
                fontSize: '14px',
                color: qualityData.overall_score >= 90 ? '#166534' : '#92400e',
                marginBottom: '8px',
                fontWeight: 600,
              ...
              <div style={{
                ...
                {qualityData.overall_score.toFixed(1)}<span style={{ fontSize: '24px', color: qualityData.overall_score >= 90 ? '#16a34a' : '#f59e0b' }}>/100</span>
              </div>
              <div style={{
                fontSize: '14px',
                color: qualityData.overall_score >= 90 ? '#15803d' : '#78350f',
                marginTop: '4px'
              }}>
                {qualityData.overall_score >= 90 ? 'Excellent quality' : 'Good quality — minor issues detected'}
              </div>
            </div>

            {/* Quality Metrics */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '24px' }}>
              <MetricCard label="Issues Found" value={qualityData.issues.length} color="#64748b" />
              <MetricCard label="High Severity" value={qualityData.high_severity_count} color="#dc2626" />
              <MetricCard label="Medium Severity" value={qualityData.medium_severity_count} color="#f59e0b" />
              <MetricCard label="Low Severity" value={qualityData.low_severity_count} color="#10b981" />
            </div>
            ...
            {(qualityData.high_severity_count + qualityData.medium_severity_count + qualityData.low_severity_count) > 0 && (
              <div style={{
                backgroundColor: '#fff',
                border: '1px solid #e2e8f0',
                borderRadius: '6px',
                  ...
                  data={[
                    {
                      ...
                      values: [
                        qualityData.high_severity_count,
                        qualityData.medium_severity_count,
                        qualityData.low_severity_count,
                      ],
                      type: 'pie',
