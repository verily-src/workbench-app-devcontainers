...
if [[ -z "${WORKSPACE_ID} || -z "${RESOURCE_ID} ]]; then
    exit 1
fi

# Get WSM endpoint URL
if ! WSM_SERVICE_URL="$(get_service_url "${SERVER}" "wsm")"; then
    exit 1
fi
...
function record_devcontainer_end() {
    if [[ $# -lt 2 || ("$2" != "0" && "$2" != "1") ]]; then
        echo "usage: record_devcontainer_end <cloudPlatform - AWS/GCP> <success - 0 or 1>"
        exit 1
    fi
    CLOUD_PLATFORM="$1"
    SUCCESS="$2"
...
function record_devcontainer_start() {
    if [[ $# -lt 1 ]]; then
        echo "usage: record_devcontainer_start <cloudPlatform - AWS/GCP>"
        exit 1
    fi
    CLOUD_PLATFORM="$1"
    payload=$(cat <<EOF
