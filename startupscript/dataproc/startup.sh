...
readonly WORKBENCH_PROXY_AGENT_STARTUP_SCRIPT="${USER_WORKBENCH_CONFIG_DIR}/start-vwb-proxy-agent.sh"
...
###################################
    if [[ "${PROXY_TYPE}" == "${PROXY_TYPE_VWB}" ]]; then
      ...
      cat <<EOF >"${WORKBENCH_PROXY_AGENT_STARTUP_SCRIPT}"
#!/bin/bash
set -o errexit
set -o nounset
...
ExecStart=${WORKBENCH_PROXY_AGENT_STARTUP_SCRIPT}
Restart=on-failure
