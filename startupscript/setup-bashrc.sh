...
if [[ "${LOG_IN}" == "true" ]]; then
  ...
  OWNER_EMAIL="$(
    ${RUN_AS_LOGIN_USER} "/usr/bin/wb workspace describe --format=json" | \
  ...
  PET_SA_EMAIL="$(
    ${RUN_AS_LOGIN_USER} "/usr/bin/wb auth status --format=json" | \
...
if [[ "${CLOUD}" == "aws" && "${LOG_IN}" == "true" ]]; then
  ...
  ${RUN_AS_LOGIN_USER} "eval \$(/usr/bin/wb workspace configure-aws) && \
